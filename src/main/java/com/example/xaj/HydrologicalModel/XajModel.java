...
public class XajModel {
    ...
    public XajModel ComputeRunoffGeneration(double[] P, double[] EI, double wu0, double wl0, double wd0) {
        int n = ParameterValidation.CheckIdenticalLength(P, EI);
     ...
     * 执行水源划分计算（需要先完成产流计算）
     *
     * @param s 产流面积上的平均自由水深 s=s0*FR0/FR
     * @param dt 时段长度
     * @return 完成水源划分计算的模型实例
     */
    public XajModel ComputeSourcePartition(double s, double dt) {
        ...
        double KSSD = (1 - Math.pow(1 - (KG + KSS), dt / 24)) / (1 + KG / KSS);     // 转化后的壤中流出流系数  M=24/dt M为时段数
        ...
        for (int i = 0; i < n; ++i) {
            ...
            double au = Smax * (1 - Math.pow(1 - s / SM, 1 / (1 + EX)));                    //与s0相应的纵坐标值au
            double FR;              // 产流面积
            double coef;
            if (pe > 0) {
                FR = (r / pe) - Imp;
                coef = (pe + au < Smax) ? SM - SM * Math.pow(1 - (pe + au) / Smax, 1 + EX) : SM;   //coef=RSSD=壤中流增加量
            } else {
                FR = 1 - Imp - Math.pow(1 - w / WM, B / (1 + B)) * (1 - Imp);
                coef = s;
            }
            double rimp = pe * Imp;//不透水面积上的净雨
            double rs = Math.max((pe + s - coef) * FR, 0.0);
            double rss = coef * KSSD * FR;
            double rg = coef * KGD * FR;
            s = coef * (1 - KSSD - KGD);
            sourcePartitionResult.Set(i, rimp, rs, rss, rg, s);
        }
        return this;
    }
    ...
    private double[] LinearReservoir(double[] R, double KK, double qr0, double dt) {
        ...
        for (int i = 0; i < R.length; ++i) {
            qr0 = qr0 * KKD + R[i] * Area / (3.6 * dt) * (1 - KKD);    //Area / (3.6 * dt)=U=流域面积F/3.6*dt
            QR[i] = qr0;
        }
        return QR;
    }
    ...
    private double[] UnitHydrograph(double[] RS, double[] RIMP, double dt) {
        int n = ParameterValidation.CheckIdenticalLength(RS, RIMP);
        double[] result = new double[n];
//        double[] UH = new double[]{0.3, 0.6, 0.1};
        double[] UH = new double[]{0.7};
        for (int i = 0; i < n; ++i) {
            double rs = RS[i];
            double rimp = RIMP[i];
            for (int j = 0; j < UH.length && i + j < n; ++j) {
                result[i + j] = result[i + j] + (rs + rimp) * UH[j] * Area / (3.6 * dt);//Area / (3.6 * dt)=U为单位转换系数，UH为无因次时段单位线
            }
        }
        return result;
    }
    ...
    private static double[] ComputeQ(double[] QRS, double[] QRSS, double[] QRG) {
        int n = ParameterValidation.CheckIdenticalLength(QRS, QRSS, QRG);
        double[] Q = new double[n];
        for (int i = 0; i < n; ++i)
        {
            if (QRS[i] + QRSS[i] + QRG[i]>0){
